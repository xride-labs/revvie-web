...
import type { PendingBusiness } from '@/entities/admin/model'
...
export default function AdminBrandsPage() {
  ...
  const { data, isLoading: loading, isFetching, refetch } = useGetAllBusinessesQuery(queryParams)
  ...
  const act = async (fn: () => Promise<unknown>, id: string, msg: string) => {
    setActioningId(id)
    try {
      await fn()
      toast.success(msg)
  ...
  const stats = {
    ...
    submitted: businesses.filter((b) => b.verification === 'SUBMITTED').length,
    approved: businesses.filter((b) => b.verification === 'APPROVED').length,
  }
        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Full CRUD — approve, reject, delete brand profiles
              </CardDescription>
            </div>
                  ...
                  businesses.map((biz) => {
                    ...
                    return (
                      <TableRow key={biz.id}>
                        ...
                        <TableCell>
                          <p className="text-sm">
                            {[biz.city, biz.region, biz.country]
                              .filter(Boolean)
                              .join(', ') || '—'}
                          </p>
                        </TableCell>
                        <TableCell>
                          <Badge className={VERIFICATION_COLORS[verification] ?? ''}>
                            {verification}
                          </Badge>
                        </TableCell>
                          ...
                          <AdminCRUDPopover
                            actions={[
                              ...
                              CRUDActionBuilders.delete(
                                () =>
                                  act(
                                    () => deleteBusiness(biz.id).unwrap(),
                                    `del-${biz.id}`,
                                    `${biz.displayName} deleted`,
                                  ),
                                false,
                                false,
                              ),
      ...
      >
        <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4 text-sm">
              <div className="flex items-center gap-4">
                <Avatar className="h-14 w-14 rounded-xl">
                  <AvatarImage
                    src={viewBiz.logoUrl ?? undefined}
                    className="object-cover"
                  />
                  <AvatarFallback className="rounded-xl text-xl">
                    {viewBiz.displayName[0]}
                  </AvatarFallback>
                </Avatar>
                <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Location">
                  {[viewBiz.city, viewBiz.region, viewBiz.country]
                    .filter(Boolean)
                    .join(', ') || '—'}
                </Field>
                <Field label="Created">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setRejectNotes((p) => ({ ...p, [viewBiz.id]: e.target.value }))
                  }
                />
              </div>
            </div>
          ...
          <DialogFooter className="gap-2">
            ...
            {viewBiz && viewBiz.verification === 'SUBMITTED' && (
              <>
                <Button
                  size="sm"
                  ...
                  onClick={() => {
                    act(
                      () => approveBusinessSubmission(viewBiz.id).unwrap(),
                      viewBiz.id,
                      'Approved',
                    )
                    setViewBiz(null)
